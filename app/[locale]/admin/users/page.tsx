  ...
  return (
    <div className="w-full pt-8 pb-6 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col h-full">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            <div className="relative group w-full md:w-80">
              ...
              <input 
                type="text" 
                className="block w-full pl-10 pr-3 py-2.5 border-none rounded-lg bg-white dark:bg-gray-800 text-nordic dark:text-white shadow-soft placeholder-nordic/30 focus:ring-2 focus:ring-mosque focus:bg-white transition-all text-sm" 
      ...
      <main className="flex-grow pb-12 w-full space-y-4">
        ...
        {/* Pagination */}
        <div className="px-6 py-4 border-t border-nordic/5 dark:border-mosque/20 flex items-center justify-between bg-clear-day/50 dark:bg-mosque/5 rounded-xl border mt-4">
          <div className="text-sm text-nordic-muted dark:text-gray-400">
            Showing <span className="font-medium text-nordic dark:text-white">{totalItems === 0 ? 0 : (page - 1) * pageSize + 1}</span> to <span className="font-medium text-nordic dark:text-white">{Math.min(page * pageSize, totalItems)}</span> of <span className="font-medium text-nordic dark:text-white">{totalItems}</span> results
          </div>
